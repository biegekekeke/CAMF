...
import argparse
from utils_slake import seed_everything, Model, VQAMed, train_one_epoch, validate, test, load_data, LabelSmoothing ,DataParallel_withLoss
import pandas as pd
import numpy as np
import torch
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description = "Finetune on SLAKE")

    parser.add_argument('--run_name', type = str, required = True, help = "run name for wandb")
    parser.add_argument('--data_dir', type = str, required = False, default = "./data_SLAKE", help = "path for data")
    ...
    parser.add_argument('--content_type', type = str, required = False, default = None,  help = "choose specific category if you want")
    ...
    parser.add_argument('--hidden_dropout_prob', type = float, required = False, default = 0.22, help = "hidden dropout probability")
    ...
    if args.content_type:
        ...
        train_df = train_df[train_df['content_type']==args.content_type].reset_index(drop=True)
        val_df = val_df[val_df['content_type']==args.content_type].reset_index(drop=True)
        test_df = test_df[test_df['content_type']==args.content_type].reset_index(drop=True)
    ...
    val_df = df[df['mode']=='train'].reset_index(drop=True)
    ...
    if args.use_pretrained:
        # keys_to_remove = ["module.transformer.trans.conv3.weight", 
        #                 "module.transformer.trans.conv4.weight", 
        #                 "module.transformer.trans.conv5.weight", 
        #                 "module.transformer.trans.conv7.weight"]
        origin_dict = torch.load(args.model_dir)
        # for key in keys_to_remove:
        #     origin_dict.pop(key, None)

        model_dict = model.state_dict() 
        model_dict.update(origin_dict) 
