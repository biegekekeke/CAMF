...
from BertCrossattlayer import BertConfig, BertPooler, CMEncoder, CMDecoder, MeanPooling, CNNPooling
...
def load_data(args):
    ...
    train_file = open(os.path.join(args.data_dir,'train.json'))
    val_file = open(os.path.join(args.data_dir,'validate.json'))
    test_file = open(os.path.join(args.data_dir,'test.json'))
    ...
    # 只取 英文部分
    trainen = [query for query in train_data if query["q_lang"] == "en"] 
    valen = [query for query in val_data if query["q_lang"] == "en"] 
    testen = [query for query in test_data if query["q_lang"] == "en"] 

    traindf = pd.DataFrame(trainen) 
    traindf['mode'] = 'train'
    valdf = pd.DataFrame(valen) 
    valdf['mode'] = 'val'
    testdf = pd.DataFrame(testen)
    testdf['mode'] = 'test' 


    traindf['img_name'] = traindf['img_name'].apply(lambda x: os.path.join(args.data_dir, 'imgs', x))
    valdf['img_name'] = valdf['img_name'].apply(lambda x: os.path.join(args.data_dir, 'imgs', x))
    testdf['img_name'] = testdf['img_name'].apply(lambda x: os.path.join(args.data_dir, 'imgs', x))

    traindf['content_type'] = traindf['content_type'].str.lower()
    valdf['content_type'] = valdf['content_type'].str.lower()
    testdf['content_type'] = testdf['content_type'].str.lower()
...
class Transformer(nn.Module):
    ...
    def forward(self, img, input_ids, token_type_ids, mask):
        ...
        out_feats = self.pooler(out_feats)  
